...
const Calendar = (() => {
  ...
  function render(events) {
    ...
    grid.innerHTML = '';
    events.forEach(ev => grid.appendChild(card(ev)));
    show('cal-list');
  }
  ...
  function card(ev) {
    ...
    if (isAllDay) {
      ...
      timeEl.textContent = fmt(ev.start.dateTime, ev.end.dateTime);
    }
  ...
  function show(id) {
    ['cal-loading','cal-error','cal-empty','cal-list'].forEach(i => {
      document.getElementById(i).classList.toggle('hidden', i !== id);
    });
  }

  function errMsg(e) {
    if (e.status === 403) return 'Calendar APIへのアクセス権限がありません。';
    if (e.status === 401 || e.message === 'TOKEN_EXPIRED') return 'セッションが切れました。再ログインしてください。';
